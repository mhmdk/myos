...
extern address kmalloc_pool_start;

#define CHUNK_SIZE  32
//also defined boot.asm
#define POOL_SIZE 16*MB
...
void initialize_kmalloc() {
	...
	memset((void*) kmalloc_pool_start, 0, POOL_SIZE);
}
...
void* kmalloc(uint32_t size) {
	...
	if (first_chunk_index > 0) {
		...
		AllocationHeader *header = (AllocationHeader*) (kmalloc_pool_start
...
void kfree(void *ptr) {
	...
	uint32_t starting_chunk = ((uint32_t) (header) - kmalloc_pool_start)
			/ CHUNK_SIZE;
