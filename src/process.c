...
Process* create_user_process(uint32_t entry_point) {
	...
	process->trap_frame->ebp = (uint32_t)process->user_stack+USER_PROCESS_DEFAULT_STACK_SIZE;
	...
	process->context = (Context*) ((uint32_t)(process->trap_frame) - sizeof(Context));
	process->context->eip = (uint32_t)(&trap_return);
	process->context->ebp = (uint32_t) process->kernel_stack + PROCESS_KERNEL_STACK_SIZE;
	process->context->esi = 0;
	process->context->edi = 0;
	process->context->ebx = 0;
